...
void xmlSerializa(Red red){
    WriteLine("Serializando datos en formato xml...");
    Utilerias.xmlGrabar(ruta,red);

    Red otraRed = new Red();
    WriteLine("Cargando datos del archivo: {0}",ruta);
    Utilerias.xmlLeer(ruta,ref otraRed);
    Reporte(otraRed);
}

void repVulnerabilidades(Red red){
    WriteLine("Reporte de vulnerabilidades");
    WriteLine("Vulnerabilidades por tipo: local");
    ...
    var vv = (red.Nodos.SelectMany(n=>n.Vulnerabilidades).Where(v=>v.Tipo=="local")).ToList();
    vv.ForEach(v=>WriteLine(v.ToString("D2",System.Globalization.CultureInfo.CurrentCulture)));
}


void reporteNodos(Red red){
    WriteLine("Reporte de nodos");
    WriteLine("Nodos SO Windows o Linux");
